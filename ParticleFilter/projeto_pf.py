...
def move_particulas(particulas, movimento):
    ...
    for particle in particulas:
        ...
        particle.move_relative(movimento)
...
def leituras_laser_evidencias(robot, particulas):
    ...
    for particle in particulas:
        ...
        for angle in leitura_particula: #os angles são os mesmos de leitura_robo, então não da erro
            prob += norm.pdf(leitura_particula[angle], loc=leitura_robo[angle], scale=15) #incerteza do robo vai de 5 a 15cm
        prob_geral += prob
        particle.w = prob
...
def reamostrar(particulas, n_particulas = num_particulas):
    ...
    for p in novas_particulas:
        p.x += norm.rvs(0, 5)
        p.y += norm.rvs(0, 5)
        p.theta += norm.rvs(0, math.radians(6))
        p.w = 1
